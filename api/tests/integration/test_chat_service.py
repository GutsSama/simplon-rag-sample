...
_GUARD_OUT_OF_SCOPE = json.dumps(
    {"in_scope": False, "needs_retrieval": False, "category": "out_of_scope"}
)
_GUARD_NO_RETRIEVAL = json.dumps(
    {"in_scope": True, "needs_retrieval": False, "category": "general"}
)
_GUARD_WITH_RETRIEVAL = json.dumps(
    {"in_scope": True, "needs_retrieval": True, "category": "support"}
)
_EVAL_ANSWER = json.dumps(
    {"score": 9.0, "decision": "answer", "rewrite_suggestion": ""}
)
_EVAL_REWRITE = json.dumps(
    {"score": 5.0, "decision": "rewrite", "rewrite_suggestion": "reformulated query"}
)
...
async def test_send_message_unknown_conversation_raises(
    service: ChatService, db_session: AsyncSession
):
    with pytest.raises(ConversationNotFoundError):
        await service.send_message(uuid.uuid4(), "Hello", db_session)


async def test_out_of_scope_saves_refusal_and_no_sources(
    service: ChatService, db_session: AsyncSession
):
...
async def test_direct_answer_no_retrieval(
    service: ChatService, db_session: AsyncSession
):
...
async def test_rag_answer_with_sources(service: ChatService, db_session: AsyncSession):
    ...
    with (
        patch("rag.rag.agent.nodes._get_llm", side_effect=llm_calls),
        patch(
            "rag.rag.retriever.pgvector_retriever.similarity_search",
            new=AsyncMock(return_value=_CHUNKS),
        ),
    ):
        result = await service.send_message(
            conv_id, "What are the refund conditions?", db_session
        )
...
async def test_escalation_after_max_retries(
    ...
    llm_calls = [
        ...
        _llm(
            _EVAL_REWRITE
        ),  # evaluate 2 → _eval_decision forces escalate (retry_count=2 >= 2)
    ]
    with (
        patch("rag.rag.agent.nodes._get_llm", side_effect=llm_calls),
        patch(
